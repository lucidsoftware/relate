...
class RelateITSpec extends Specification with Db {
  ...
  "insert" should {
    ...
    "work in the basic case (uses select to test)" in withConnection { implicit connection =>
      ...
      (entries.size must ===(1)) and (entries(0).name must beEqualTo(pokemonName)) and (entries(
        0
      ).description must ===(pokemonDesc))
    }
    ...
    "work for multi-insert" in withConnection { implicit connection =>
     ...
     """.asList(pokemonParser)
      ...
      (pokemon.size must ===(records.size)) and (levelAndTrainerIdFromDb must ===(levelAndTrainerIdFromTest))
    }
    ...
    //   ids.size must beEqualTo(newEntries.size)
    // }
  }
  ...
  "select" should {
    "work with asSingle (also tests retrieving null value)" in withConnection { implicit connection =>
      ...
      (pokemon.id must beEqualTo(7)) and (pokemon.pokedexId must beEqualTo(4)) and (pokemon.level must beEqualTo(3)) and (pokemon.trainerId must beNone)
    }
    ...
    "work with asSet" in withConnection { implicit connection =>
      ...
      (pokemon.size must beEqualTo(1)) and (pokemon must contain(squirtle))
    }
    ...
    "work with asSeq" in withConnection { implicit connection =>
      ...
      (pokemon.size must beEqualTo(2)) and (pokemon must contain(squirtle)) and (pokemon must contain(pikachu))
    }
    ...
    "work with asIterable" in withConnection { implicit connection =>
      ...
      (iterableAsList must contain(names(0))) and (iterableAsList must contain(
        names(1)
      )) and (iterableAsList must contain(names(2)))
    }
    ...
    "work for asList" in withConnection { implicit connection =>
      ...
      (pokemon.size must beEqualTo(2)) and (pokemon must contain(jigglypuff)) and (pokemon must contain(magikarp))
    }
    ...
    "work for asMap" in withConnection { implicit connection =>
      ...
      (pokemon(wartortle.name) must beEqualTo(wartortle.description)) and (pokemon(
        blastoise.name
      ) must beEqualTo(blastoise.description))
    }
    ...
    "work for asIterator" in withConnection { implicit connection =>
      ...
      (pokemon.size must beEqualTo(2)) and (pokemon must contain(wartortle)) and (pokemon must contain(blastoise))
    }
    ...
    "work using expand for the IN clause" in withConnection { implicit connection =>
      ...
      (pokemonNames must contain("Squirtle")) and (pokemonNames must contain(
        "Wartortle"
      )) and (pokemonNames must contain("Blastoise"))
    }
    ...
    "work with null" in withConnection { implicit connection =>
      ...
      result must beEqualTo((1, None, 4))
    }
  }
