...
class RelateITSpec extends Specification with Db {
  ...
  "insert" should {
    ...
    "work in the basic case (uses select to test)" in withConnection { implicit connection =>
      ...
      (entries.size must_== 1) and (entries(0).name must_== pokemonName) and (entries(
        0
      ).description must_== pokemonDesc)
    }
  ...
  "select" should {
    ...
    "work for asMap" in withConnection { implicit connection =>
      ...
      (pokemon(wartortle.name) must_== wartortle.description) and (pokemon(
        blastoise.name
      ) must_== blastoise.description)
    }
